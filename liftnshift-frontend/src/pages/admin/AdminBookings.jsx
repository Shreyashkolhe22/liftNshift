...
function AssignModal({ booking, onClose, onAssigned }) {
  ...
  const [aiLoading,   setAiLoading]   = useState(false);  // ← only once
  const [assigning,   setAssigning]   = useState(false);
  const [error,       setError]       = useState("");
  const [loadingData, setLoadingData] = useState(true);

  // Load available trucks + drivers
  useEffect(() => {
    async function load() {
      setLoadingData(true);
      try {
  ...
  async function handleAiRecommend() {
    ...
    try {
      const res = await fetch(
        `${API}/api/admin/bookings/${booking.id}/recommend-truck`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (!res.ok) throw new Error("AI recommendation failed");
      const data = await res.json();
      setAiRec(data);
      if (data.availableTrucks)  setTrucks(data.availableTrucks);
      if (data.availableDrivers) setDrivers(data.availableDrivers);
    } catch (e) {
  setError(
    e.message?.includes("429") || e.message?.includes("unavailable")
      ? "AI is temporarily rate limited. Wait 30 seconds and try again."
      : "AI recommendation failed. Please select truck manually."
  );
}
    setAiLoading(false);
  }
  ...
  const confColor = aiRec?.confidence === "HIGH"
    ? "#34D399" : aiRec?.confidence === "MEDIUM" ? "#FBBF24" : "#F87171";
      ...
      <div className="assign-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="am-summary">
          ...
          <div className="am-summary-row">
            ...
            <span className="am-summary-val" style={{color:"#F47B20",fontWeight:600}}>
        ...
        {loadingData ? (
          ...
          <>
            {/* ── TRUCK SELECTOR with AI button ── */}
            <div className="am-section">
              <div style={{
                display:"flex", justifyContent:"space-between",
                alignItems:"center", marginBottom:10
              }}>
                <p className="am-section-label" style={{margin:0}}>
                  Select Truck
                  {aiRec && (
                    <span className="am-rec-badge"
                      style={{background:`${confColor}18`, color:confColor, marginLeft:8}}>
                ...
                <button
                  ...
                  disabled={aiLoading}
                >
                  {aiLoading ? "⏳ Analyzing…" : aiRec ? "🤖 Re-analyze" : "🤖 Get AI Recommendation"}
                </button>
              </div>
              ...
              {trucks.length === 0 ? (
                ...
                <div className="am-options">
                  {trucks.map(t => (
                    <label key={t.id}
                      className={`am-option
                        ${selTruck === String(t.id) ? "am-option-sel" : ""}
            ...
            {aiRec && (
              <div className="am-ai-result">
                ...
                <div className="am-ai-truck">
                  ...
                  <strong style={{color:"#F47B20", marginLeft:6}}>
                    {aiRec.recommendedTruckSize}
                  </strong>
                  <span style={{color:"#606060", marginLeft:8}}>
                    ~{aiRec.estimatedLoadPercent}% load
                  </span>
                </div>
                          ...
                          <td>
                            <div className="actions">
                              ...
                              <button className="btn btn-danger"
                                onClick={() => setConfirm({
                                  type:"delete", bookingId:b.id, label:`#${b.id}`
                                })}>
                                Del
                              </button>
                            </div>
                          ...
                          <tr key={`${b.id}-items`}>
                            <td colSpan={8} style={{background:"#202020",padding:"12px 18px"}}>
                              <p style={{
                                ...
                                letterSpacing:".12em", color:"#606060", marginBottom:8
                              }}>
                              ...
                              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                                {b.items.map(i => (
                                  <div key={i.id} style={{
                                    ...
                                    {i.size && (
                                      <span style={{color:"#606060",marginLeft:6}}>
                                        · {i.size}
                                      </span>
                                    )}
                                    <span style={{color:"#606060",marginLeft:6}}>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-sub">
              {confirm.type === "delete"
                ...
                : `Change Booking ${confirm.label} to "${confirm.newStatus?.replace("_"," ")}"?`}
            </p>
...
.am-section-label{font-size:.68rem;text-transform:uppercase;letter-spacing:.14em;color:var(--muted);display:flex;align-items:center;gap:8px}
...
.am-ai-btn{background:rgba(52,211,153,.1);border:1px solid rgba(52,211,153,.25);color:#34D399;padding:6px 14px;border-radius:8px;font-family:'DM Sans',sans-serif;font-size:.8rem;font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap}
.am-ai-btn:hover:not(:disabled){background:rgba(52,211,153,.2);border-color:#34D399}
.am-ai-btn:disabled{opacity:.5;cursor:not-allowed}
